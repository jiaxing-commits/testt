...
RC openPageFile (char *fileName, SM_FileHandle *fHandle){
    ...
    if (fopen(fileName, "r+") == NULL) return RC_FILE_NOT_FOUND;
        ...
        // update filename and current position
...
RC writeBlock (int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage) {
    ...
    if (pageNum == 0) {
        ...
        fseek(file, (PAGE_SIZE * pageNum), SEEK_SET);

        for (int i = 0; i < PAGE_SIZE; i++) {
            if (feof(file)) appendEmptyBlock(fHandle); // end of file so append new empty block
    ...
    return RC_OK; // write block successful
}
...
RC writeCurrentBlock (SM_FileHandle *fHandle, SM_PageHandle memPage) {
    ...
    fseek(file, fHandle->curPagePos, SEEK_SET); // find page
    fwrite(memPage, sizeof(char), strlen(memPage), file); // write page to file
    fHandle->curPagePos = ftell(file); // store current page position
    fclose(file); // close file

    return RC_OK; // writing to file successful
}
...
RC ensureCapacity (int numberOfPages, SM_FileHandle *fHandle) {
    ...
    while (fHandle->totalNumPages <= numberOfPages) appendEmptyBlock(fHandle); // need more space, append empty block
